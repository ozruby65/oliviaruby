...
public class Hobbemoun implements Comparable<Hobbemoun> {
   ...
   * and the first and second IDs set to 0. The primary and secondary types are set to the given
   * primary and secondary types, respectively.
   *
  ...
  public int compareTo(Hobbemoun otherHobbemoun) {
    // First: Compare the primary types, if they are different
    if (this.getPrimaryType() != otherHobbemoun.getPrimaryType()) {
      int primaryComparison = Integer.compare(this.getPrimaryType().getValue(),
          otherHobbemoun.getPrimaryType().getValue());

      return primaryComparison;
    }


    // Second: Compare the secondary types, if they are different and both not null
    if (this.getSecondaryType() != null && otherHobbemoun.getSecondaryType() != null
        && this.getSecondaryType() != otherHobbemoun.getSecondaryType()) {

      int secondaryComparison = Integer.compare(this.getSecondaryType().getValue(),
    ...
    // Third: Compare the names of the hobbemouns.
    if (this.getName() != null && otherHobbemoun.getName() != null) {
      return this.getName().compareTo(otherHobbemoun.getName());
    }

    // If all elements of the hobbemoun are null, return 0
    return 0;
